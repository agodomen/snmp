...
public class SnmpData {
	...
	public static void snmpGet(String ip, String community, String oid) {
		...
		try {
			...
			if (response == null) {
				...
				System.out.println("返回的 pdu 大小是： " + response.size());
			...
			System.out.println("SNMP获取一个OID值结束 !");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void snmpGetList(String ip, String community,
		...
		try {
			...
			if (response == null) {
				...
				System.out.println("返回的 pdu的大小：" + response.size());
				for (int i = 0; i < response.size(); i++) {
					VariableBinding vb = response.get(i);
					System.out.println(vb.getOid() + " = " + vb.getVariable());
				}
			...
			System.out.println("SNMP获取一个OID结束!");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("SNMP Get Exception:" + e);
		} finally {
	...
	public static void snmpAsynGetList(String ip, String community,
		...
		try {
			...
			pdu.setType(PDU.GET);
			ResponseEvent respEvent = snmp.send(pdu, target);
			System.out.println("PeerAddress值为:" + respEvent.getPeerAddress());
			PDU response = respEvent.getResponse();
	...
	public static void setPDU(String ip, String community, String oid)
		...
		Snmp snmp = null;
		PDU pdu = new PDU();
		pdu.add(new VariableBinding(new OID(oid), new OctetString("public")));
		pdu.setType(PDU.SET);
		DefaultUdpTransportMapping transport = new DefaultUdpTransportMapping();
		snmp = new Snmp(transport);
		snmp.listen();
